...
BOOL CAboutDlg::OnInitDialog()
{
	...
	strBuffer.Replace("<VERSION>", "" );
	strBuffer.Replace("<RELEASE>", "" );
	strBuffer.Replace("<XNOTESTOPWATCHSUPPORT>", "\n" );
	strBuffer.Replace("<BUGFIXES>", "Bugfixes and additional features added by Gregory P. Spata.\n" );
	strBuffer.Replace("<COPYRIGHT-1>", "\nCopyright © 2013 UXC Recorder\n" );
	strBuffer.Replace("<COPYRIGHT-2>", "" );
	//***UXC
...
//#endif
	...
	cAudioFormat.m_iRecordAudio = 0;

	//if(cfg->exists("Video"))
	//	cVideoOpts.Read(cfg->lookup("Video"));
	cVideoOpts.m_bAutoAdjust = false;
	cVideoOpts.m_iFramesPerSecond = 10;
	cVideoOpts.m_iKeyFramesEvery = 10;
	cVideoOpts.m_iTimeLapse = 100;  //Capture Frame rate (ms)
	cVideoOpts.m_iCompQuality = 9000;
	//cVideoOpts.m_iTimeShift = 100;   //Not Needed
	//cVideoOpts.m_bRestrictVideoCodecs = true;   //Not Needed
	cVideoOpts.m_iSelectedCompressor = 0;   
	cVideoOpts.m_dwCompfccHandler = 1129730893;   //Microsoft Video 1
	
	//if(cfg->exists("Cursor"))
	//	CamCursor.Read(cfg->lookup("Cursor"));
	CamCursor.Highlight(true);
	CamCursor.HighlightClick(true);
	CamCursor.Record(true);
	CamCursor.ClickLeftColor(-1593901056);
	CamCursor.ClickRightColor(-1610612481);

	//if(cfg->exists("Program"))
	//	cProgramOpts.Read(cfg->lookup("Program"));
	cProgramOpts.m_bAutoNaming = false;
	cProgramOpts.m_bFlashingRect = false;
	cProgramOpts.m_bSaveSettings = false;
	cProgramOpts.m_bMinimizeOnStart = false;
	cProgramOpts.m_iTempPathAccess = 1;  //1 = use app path
	cProgramOpts.m_iRecordingMode = 0;
	cProgramOpts.m_iViewType = 2;  //2 = button view;  0 = normal view
	cProgramOpts.m_iLaunchPlayer = 0;  //0 = do not play AVI file
	...
	cRegionOpts.m_iCaptureMode = 4;  //4 = full screen

	//if(cfg->exists("Caption"))
	//	cCaptionOpts.Read(cfg->lookup("Caption"));
	cCaptionOpts.m_bAnnotation = false;

	//if(cfg->exists("TimeStamp"))
	//	cTimestampOpts.Read(cfg->lookup("TimeStamp"));
	cTimestampOpts.m_bAnnotation = false;

	//if(cfg->exists("XNote"))
	//	cXNoteOpts.Read(cfg->lookup("XNote"));
	cXNoteOpts.m_bAnnotation = false;

	//if(cfg->exists("Watermark"))
	//	cWatermarkOpts.Read(cfg->lookup("Watermark"));
	cWatermarkOpts.m_bAnnotation = false;

	//if(cfg->exists("Producer"))
	//	cProducerOpts.Read(cfg->lookup("Producer"));
	cProducerOpts.m_bLaunchHTMLPlayer = false;
	cProducerOpts.m_bLaunchPropPrompt = false;
	cProducerOpts.m_bDeleteAVIAfterUse = false;
	// *** UXC 

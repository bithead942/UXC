...
void CRecorderView::LoadSettings()
{
    ...
    if (ver < 1.799999)
    {
        ...
        if ((cHotKeyOpts.m_RecordStart.m_vKey == VK_MENU)
            ...
            cHotKeyOpts.m_RecordStart.m_vKey = VK_UNDEFINED;    //UXC:  Remove VK_F8
        ...
        if ((cHotKeyOpts.m_RecordEnd.m_vKey == VK_MENU)
            ...
            cHotKeyOpts.m_RecordEnd.m_vKey = VK_UNDEFINED;  //UXC:  Remove VK_F9
        ...
        if ((cHotKeyOpts.m_RecordCancel.m_vKey == VK_MENU)
            ...
            cHotKeyOpts.m_RecordCancel.m_vKey = VK_UNDEFINED;   //UXC:  Remove VK_F10
...
bool CRecorderView::RecordVideo(CRect rectFrame, int fps, const char *szVideoFileName)
{
	...
	//*** UXC
    // Create output string for .TXT file
	// current date/time based on current system
    time_t now = time(0);
	TCHAR computernameVal[MAX_COMPUTERNAME_LENGTH + 2];
	DWORD computernameSize;
	computernameSize = sizeof computernameVal - 1;
	GetComputerName(computernameVal, &computernameSize);

	TCHAR usernameVal [ UNLEN + 1 ];
    DWORD usernameSize = UNLEN + 1;
	GetUserName( (TCHAR*)usernameVal, &usernameSize );

    char cTmp[128] = "";
    sprintf( cTmp, "***UXC Recorder Session*** \n\nRecorded On:  %sComputer Name: %s \nUser Name: %s", ctime(&now), computernameVal, usernameVal );
	//*** UXC
